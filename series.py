...
class Series:
    ...
    def __init__(self, directory, file_pattern,
        ...
        self.file_pattern_format = file_pattern.format(number='{number:' + number_format + 'n}', garbage='{garbage}')
    ...
    def set_entries_from_directory(self):
        ...
        regex_number = r'(\d+)(v\d)?'.join([r'.*?'.join([re.escape(q) \
                ...
                for p in pattern.format(number='{number}', garbage='{garbage}').split('{number}')])
    ...
    def set_entries_from_transmission(self, torrents):
        ...
        regex_number = r'(\d+)(v\d)?'.join([r'.*?'.join([re.escape(q) \
                for q in p.split('{garbage}')]) \
                for p in self.file_pattern.format(number='{number}', garbage='{garbage}').split('{number}')])
